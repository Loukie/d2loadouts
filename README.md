...
1. Open `index.html` in any browser (just double-click it — no install, no server).
2. **Drop in your `settings.json`** (the mod's character file).
3. **Pick a Guardian** — Titan, Hunter, or Warlock.
4. **Edit** anything:
...
5. **Download** and drop the file back where your `settings.json` lives, then
   launch the game.
...
- **Saves are compact.** The mod reads `settings.json` into a fixed 64 KB buffer
  and rejects anything larger ("Problem reading game content"), so the tool writes
  compact JSON (~30 KB) and refuses to save a file over the limit.
